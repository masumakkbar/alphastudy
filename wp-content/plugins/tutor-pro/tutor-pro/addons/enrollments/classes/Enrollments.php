...
class Enrollments {
	...
	public function tutor_enroll_bulk_student() {
		...
		$csv_students   = $request->csv_students;
		...
		if ( ! empty( $csv_students ) ) {
			...
			if ( 0 === count( $student_ids ) ) {
				$this->response_bad_request( __( 'Invalid or Duplicate data found in csv file', 'tutor-pro' ) );
			}
		}
			...
			foreach ( $object_ids as $object_id ) {
				...
				foreach ( $student_ids as $student_id ) {
					...
					if ( $is_already_enrolled ) {
						...
						$failed_enrollments[] = $failed_user;
						continue;
					}
	...
	public function get_student_ids_from_csv( $students_from_csv, $student_ids ) {
		...
		if ( is_array( $students_from_csv ) && count( $students_from_csv ) ) {
			...
			foreach ( $students_from_csv as $student ) {
				...
				$user_login = trim( sanitize_user( wp_slash( $student->first_name . ' ' . $student->last_name ), true ) );

				if ( email_exists( $student->email ) && username_exists( $user_login ) ) {
					...
					$student_id = wp_create_user( $user_name, $password, $user_email );
		...
		try {
			...
			if ( is_a( $query, 'WP_Query' ) ) {
				foreach ( $query->get_posts() as $post ) {
					...
					$course_data           = array_merge( Course::get_card_data( $post ), array( 'maximum_students' => $maximum_students ) );
	...
	public function restrict_enrollment( $btn, $course_id ) {
		$user_id = get_current_user_id();
		if ( tutor_utils()->is_enrolled( $course_id, $user_id, false ) || tutor()->course_post_type !== get_post_type( $course_id ) ) {
			return $btn;
		}
