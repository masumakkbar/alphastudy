...
class Enrollment_Expiry {
		...
		if ( is_array( $results ) && count( $results ) ) {
			$expired_list = array_filter(
				...
				function( $item ) {
					...
					return ! $expiry_date ? true : ( $expiry_date && time() > $expiry_date ? false : true );
				}
			);
	...
	public static function is_expired( $status, $enrollment ) {
		...
		if ( $custom_expiry_date ) {
			$is_expired = time() > $custom_expiry_date;
		} else {
			$is_expired = $default_expiry_date && time() > $default_expiry_date;
		}
